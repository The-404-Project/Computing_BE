...
async function generateSuratKeteranganAktif(req, res) {
  const {
    nomor_surat,
    nim,
    keperluan,
  ...
  try {
    const existing = await service.findDokumenByNomor(String(nomor_surat))
    if (existing) {
      return res.status(409).json({ message: `Surat dengan nomor registrasi ${nomor_surat} sudah ada`, file: existing.file_name })
    }
    const m = await service.findMahasiswaByNim(String(nim))
    if (!m) {
      return res.status(404).json({ message: 'Mahasiswa tidak ditemukan' })
    }
    const statusLower = String(m.status || '').toLowerCase().trim()
    ...
    const templatePath = path.join(
      __dirname,
      '../../templates/surat_templates/template_surat_keterangan_mahasiswa_aktif.docx'
    )
...
module.exports.generateSuratKeteranganAktif = generateSuratKeteranganAktif
...
async function generateSuratKeterangan(req, res) {
  ...
  try {
    ...
    if (existing) {
      return res.status(409).json({ message: `Surat dengan nomor registrasi ${body.nomor_surat} sudah ada`, file: existing.file_name })
    }
    const m = await service.findMahasiswaByNim(String(nim))
    if (!m) {
      return res.status(404).json({ message: 'Mahasiswa tidak ditemukan' })
